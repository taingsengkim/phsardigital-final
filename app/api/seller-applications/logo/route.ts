...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${BASE_URL}/api/v1/seller-applications/logo`, {
      method: "POST",
      headers: {
    ...
    console.error("Error uploading seller logo:", err);
    return NextResponse.json(
      { message: err?.message || "Failed to upload seller logo" },
      { status: 502 }
    );
  }
}

...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${BASE_URL}/api/v1/subscriptions/me`, {
      headers: {
    ...
    return NextResponse.json(
      { message: err?.message || "Failed to fetch seller subscription" },
      { status: 502 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("Error subscribing seller:", err);
    return NextResponse.json(

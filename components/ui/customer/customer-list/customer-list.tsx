...
const customers: Customer[] = [
  { id: 1, name: "Chelsie Haley", email: "chelsie@ui8.net", phone: "+855 966931349", avatar: "/picture/sengkim.jpg", status: "active" },
  { id: 2, name: "Filomena Fahey", email: "fahey.designer@robot.co", phone: "+855 966931349", avatar: "/picture/menghor.jpg", status: "active" },
  { id: 3, name: "Orion Luettgen", email: "orion88@yahoo.com", phone: "+855 966931349", avatar: "/picture/bunleang.jpg", status: "active" },
...
export function CustomerList() {
  ...
  return (
    <section className="flex min-h-[calc(100vh-104px)] flex-col bg-[#f7f7f8] px-[28px] py-[30px] text-[#27282b] sm:px-[38px]">
      <h1 className="mb-[24px] text-[32px] font-bold leading-none tracking-[-0.8px]">Customer list</h1>
      <div className="flex-1 rounded-[10px] bg-white px-[18px] pb-[12px] pt-[18px] shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
        <div className="flex flex-wrap items-center gap-[16px] pb-[28px]">
          ...
          <label className="relative w-full max-w-[345px] sm:ml-[7px]"><Search className="absolute left-[13px] top-1/2 size-[18px] -translate-y-1/2 text-[#75808c]" /><input value={query} onChange={(event) => setQuery(event.target.value)} type="search" placeholder="Search by name or email" className="h-[39px] w-full rounded-[10px] border-0 bg-[#f4f4f5] pl-[40px] pr-[14px] text-[13px] outline-none focus:ring-2 focus:ring-[#8068e8]/25" /></label>
          <div className="ml-auto flex items-center gap-[4px] rounded-[9px] bg-white"><button type="button" onClick={() => setFilter("active")} className={cn("h-[42px] rounded-[9px] px-[20px] text-[12px] font-semibold text-[#6d747c]", filter === "active" && "bg-[#f1f1f2] text-[#25272a]")}>Active</button><button type="button" onClick={() => setFilter("new")} className={cn("h-[42px] rounded-[9px] px-[20px] text-[12px] font-semibold text-[#6d747c]", filter === "new" && "bg-[#f1f1f2] text-[#25272a]")}>New</button><button type="button" aria-label="More filters" className="ml-[6px] grid size-[42px] place-items-center rounded-[9px] border border-[#e1e3e6] text-[#707982]"><ListFilter className="size-[19px]" /></button></div>
        </div>
        ...
        <div className="min-w-[720px]">
          ...
          {visible.map((customer, index) => (
            <div key={customer.id} className={cn("grid min-h-[100px] grid-cols-[42px_minmax(220px,1.25fr)_minmax(230px,1.3fr)_120px_120px_100px] items-center border-b border-[#eceef0] px-[0] py-[12px] last:border-0", index === 1 && filter === "active" && "my-[8px] rounded-[9px] border-b-0 bg-[#f1f1f2] px-[10px]")}>

...
import { Bell, ChevronDown, LogOut, MessageCircle, Moon, Search, Settings, ShoppingBag, Sparkles, Star, Sun, User } from "lucide-react"
...
export function NavTopbar() {
  ...
  const notifications: SellerNotification[] = [
    ...
    ...conversations.filter((conversation) => conversation.unreadCount > 0).map((conversation) => ({
      ...
      detail: conversation.lastMessage || `${conversation.unreadCount} unread message(s)`,
  ...
  const eventUnread = notifications.filter((notification) => notification.kind !== "message" && new Date(notification.at).getTime() > seenAt).length
  const messageUnread = conversations.reduce((sum, conversation) => sum + conversation.unreadCount, 0)
  ...
  const pageTitle = (() => {
    ...
    if (pathname === "/seller-dashboard/products/drafts") return "Draft Products"
    if (pathname === "/seller-dashboard/products/released") return "Released Products"
    if (pathname === "/seller-dashboard/products/new") return "New Product"
    if (pathname.startsWith("/seller-dashboard/products/new")) return "Edit Product"
    if (pathname === "/seller-dashboard/products/dashboard") return "Product Overview"
    ...
    if (pathname.startsWith("/seller-dashboard/message")) return "Message Center"
    ...
    <header className="sticky top-0 z-50 flex min-h-[70px] shrink-0 items-center gap-3 border-b border-[#eeeeee] bg-white px-4 py-2.5 dark:border-border dark:bg-background sm:px-6 lg:px-5">
      ...
      <div className="min-w-0">
        <h1 className="truncate text-[18px] font-bold leading-tight text-[#352b27] dark:text-foreground sm:text-[20px]">
          {pageTitle ?? <>Welcome Back, {firstName} <span aria-hidden="true">👋</span></>}
        </h1>
      </div>
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <DropdownMenu onOpenChange={(open) => { if (open) setSeenAt(Date.now()) }}>
          <DropdownMenuTrigger asChild>
            <Button type="button" variant="ghost" size="icon" aria-label={`${unreadCount} notifications`} className={`${iconButton} relative`}>
              <Bell className="size-[18px]" strokeWidth={1.8} />
              {unreadCount > 0 && <span className="absolute -right-1 -top-1 grid min-w-5 place-items-center rounded-full bg-red-500 px-1 text-[10px] font-bold leading-5 text-white ring-2 ring-white dark:ring-card">{unreadCount > 99 ? "99+" : unreadCount}</span>}
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-[340px] rounded-2xl p-2 shadow-xl">
            <DropdownMenuLabel className="flex items-center justify-between px-2 py-2"><span className="font-bold">Notifications</span><span className="text-xs font-normal text-muted-foreground">{unreadCount} unread</span></DropdownMenuLabel>
            <DropdownMenuSeparator />
            {notifications.length === 0 ? (
              <div className="px-4 py-10 text-center text-sm text-muted-foreground">No notifications yet.</div>
            ) : notifications.slice(0, 8).map((notification) => {
              const Icon = notification.kind === "order" ? ShoppingBag : notification.kind === "review" ? Star : MessageCircle
              return (
                <DropdownMenuItem key={notification.id} asChild className="cursor-pointer rounded-xl p-0">
                  <Link href={notification.href} className="flex items-start gap-3 px-3 py-3">
                    <span className="mt-0.5 grid size-9 shrink-0 place-items-center rounded-full bg-primary/10 text-primary"><Icon className="size-4" /></span>
                    <span className="min-w-0 flex-1"><span className="block text-sm font-semibold">{notification.title}</span><span className="block truncate text-xs text-muted-foreground">{notification.detail}</span></span>
                    <span className="shrink-0 text-[10px] text-muted-foreground">{relativeTime(notification.at)}</span>
                  </Link>
                </DropdownMenuItem>
              )
            })}

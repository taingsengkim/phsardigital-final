...
export function ProductTableToolbar({ query, onQueryChange, createdDate, onCreatedDateChange, columns, visibleColumns, onToggleColumn, searchPlaceholder = "Search products..." }: {
  query: string
  ...
  return <div className="mb-6 flex flex-wrap items-center gap-3  border-[#e4e7eb] bg-white p-5">
    <label className="relative min-w-[260px] flex-1 lg:max-w-[350px]"><Search className="absolute left-4 top-1/2 size-5 -translate-y-1/2 text-[#657184]" /><input value={query} onChange={(event) => onQueryChange(event.target.value)} type="search" placeholder={searchPlaceholder} className="h-14 w-full rounded-xl border border-[#dde1e6] bg-white pl-12 pr-4 text-base outline-none focus:border-[#8068e8] focus:ring-2 focus:ring-[#8068e8]/15" /></label>
